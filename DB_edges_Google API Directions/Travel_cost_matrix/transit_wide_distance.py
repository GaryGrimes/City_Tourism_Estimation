...
if __name__ == '__main__' and query_flag:
    ...
    filename = 'travel_distance_matrix_driving.csv'
    temp_dist = pd.read_csv(os.path.join(os.path.dirname(__file__), 'Database', 'Final', filename), index_col=0)

    size_temp = temp_dist.shape
    # create time matrix and cost matrix
    size = [47, 47]
    Dist_matrix = pd.DataFrame(np.zeros(size), dtype=int)
    Dist_matrix.loc[0:36, 0:36] = np.array(temp_dist)  # the index are in mess (changed to string)
    ...
    for daytime in daytimes:
        ...
        for mode in modes:
                    ...
                    if i > j:  # only query the upper-triangular area of the matrix
                        ...
                        for _origin in _origins:
                            for _destination in _destinations:
                                ...
                                try:
                                    tdist = query_result['legs'][0]['distance']['value']
                                except:
                                    print(
                        ...
                        edge_list[i][j].travel_distance = min(travel_distance)

                        # write into df
                        Dist_matrix.loc[i, j] = edge_list[i][j].travel_distance
            # %% post processing
            # 一些没结果的od，手动填补
            ...
            # travel time output
            name_tempt = daytime + '_' + mode + '_' + 'wide_distance_matrix' + '.csv'
            Dist_matrix.to_csv(os.path.join(os.path.dirname(__file__), 'Database', 'Final', name_tempt))
