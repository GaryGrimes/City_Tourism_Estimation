...
if __name__ == '__main__':
    ...
    for name in areas:
        ...
        for node in nodes:
            ...
            for type in types:
                print('Parsing %ss around node: %s, distance as %d' % (type, ','.join([str(x) for x in node]),
                                                                       radius))
                place = api.search_places_by_coordinate(','.join([str(x) for x in node]), str(radius), type)
        ...
        Data_wide.to_csv('./Temple and shrine/' + str(name) + '_wide.csv', index=False)
        ...
        Data.to_csv('./Temple and shrine/' + str(name) + '.csv', index=False)
